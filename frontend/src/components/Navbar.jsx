...
import { LogOut, LayoutDashboard, UtensilsCrossed, LogIn, ClipboardList, ChefHat, Sparkles } from 'lucide-react';
          ...
          {user ? (
            <>
              ...
              {user.role === 'admin' && (
                <Link
                  to="/dashboard"
                  className="flex items-center gap-1.5 text-xs sm:text-sm font-medium text-dark-200 hover:text-white transition-colors animate-pulse-subtle"
                >
                  <LayoutDashboard size={14} className="text-brand-400 sm:w-4 sm:h-4" />
              ...
              {user.role === 'waiter' && (
                <Link
                  to="/waiter"
                  className="flex items-center gap-1.5 text-xs sm:text-sm font-medium text-dark-200 hover:text-white transition-colors"
                >
                  <ClipboardList size={14} className="text-brand-400 sm:w-4 sm:h-4" />
              ...
              {user.role === 'kitchen' && (
                <Link
                  ...
                  className="flex items-center gap-1.5 text-xs sm:text-sm font-medium text-dark-200 hover:text-white transition-colors"
                >
                  <ChefHat size={14} className="text-brand-400 sm:w-4 sm:h-4" />
                  <span className="hidden sm:inline">Kitchen Board</span>
                </Link>
              )}

              {/* User Profile Info & Logout */}
              <div className="h-5 w-[1px] bg-dark-800 hidden sm:block"></div>
